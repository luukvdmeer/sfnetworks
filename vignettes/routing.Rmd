...
## Creating isochrones and isodistances
...
The workflow presented above is implemented in a spatial morpher function `to_spatial_neighborhood()`, which can be used inside the `tidygraph::convert()` verb to filter only those nodes that can be reached within a given travel cost from the given origin node. It accepts any geospatial point (provided as sf or sfc object) as origin, and will snap that point to its nearest node before calculating the neighborhood. Of course it is also possible to directly provide an existing node index. The edge weights to be used for defining proximity can be provided in the same manner as with the shortest path calculation functions.

In the example below we calculate neighborhoods of point $p$ with different thresholds. Since we don't specify the `weights` argument explicitly, the *weight* column containing the geographic edge lengths is used automatically for distance calculations.
...
for (i in c(1:10)) {
  iso = convert(net, to_spatial_neighborhood, p, thresholds[i])
  plot(iso, col = palette[i], add = TRUE)
}

plot(p, pch = 8, cex = 2, lwd = 2, add = TRUE)
```

## Including custom routing profiles
