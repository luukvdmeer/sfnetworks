...
```{r}
set.seed(428)
rdm = st_sample(st_as_sfc(st_bbox(roxel)), 10, type = "random") %>%
  st_transform(3035)
```
...
Now that we know the order at which each location will be visited, we can compute the shortest path between each location and its subsequent one, until we reach the starting point again. We can do this by using a for loop and the `st_network_paths()` function.

```{r}
l = list()
for(i in 1:length(idx_tour)-1){
  l[i] = st_network_paths(
    net,
    from = idx_tour[i],
    to = idx_tour[i+1]
  )$node_paths
}
# Add the path from the last location on the Tour list to the first location.
l = append(
  l,
  st_network_paths(
    net,
    from = idx_tour[length(idx_tour)],
    to = idx_tour[1]
  )$node_paths
)
```
