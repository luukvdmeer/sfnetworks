...
Another way to calculate shortest paths, which fits smoothly in the tidygraph style of working, is by using the `to_spatial_shortest_paths` morpher function. This will subset the original network to only contain those nodes and edges that appear in a shortest path between two nodes. See the [Spatial morphers]() vignette for details.
