...
The `to_spatial_simple()` morpher removes loop edges and parallel edges from the original network. The morphed state contains a single sfnetwork. See the [Network pre-processing and cleaning]() vignette for more details.
...
network is preserved by concatenating the incident edges of each removed pseudo node. The morphed state contains a single sfnetwork. See the [Network pre-processing and cleaning]() vignette for more details.

### to_spatial_subset

The `to_spatial_subset()` morpher takes a subset of the network by applying a spatial filter. A spatial filter is a filter on a geometry list column based on a spatial predicate. The morphed state contains a single sfnetwork. We can use this for example to spatially join information *only* to a spatial subset of the nodes in thet network. A tiny example just to get an idea of how this would work:
