...
pois = data.frame(poi_type = c("bakery", "butcher", "bar"),
                  x = c(0, 0.6, 0.4), y = c(0.3, 0.3, 0.4)) %>%
  st_as_sf(coords = c("x", "y"))

blended = st_network_blend(net, pois)
blended_with_tolerance = st_network_blend(net, pois, tolerance = 0.3)
...
plot(l1)
plot(l2, col = "grey", add = TRUE)
plot(st_network_blend(net, p, tolerance = 0), add = TRUE)
plot(l1)
plot(l2, col = "grey", add = TRUE)
plot(st_network_blend(net, p, tolerance = 1e-10), add = TRUE)
```
