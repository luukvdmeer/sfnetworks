...
The first family of functions in `{sfnetworks}` have names formatted as `st_network_*`, adopting the naming conventions of `{sf}`. These functions always take a `sfnetwork` object as first input, and use or compute some set of spatial features. The returned object can either be a set of individual spatial features computed from that network, the network itself with other spatial features merged into it, or another object (e.g., a matrix) that is the result of a computation utilizing the network and a set of spatial features. This covers a large range of use-cases. Examples include drawing isochrone or isodistance polygons around nodes, finding geographic shortest paths between pairs of nodes, computing cost matrices of travel between nodes, and adding external point data as new nodes to the network. See the vignettes [Routing on spatial networks]() and [Spatial joins and filters]() for detailed examples on this.
