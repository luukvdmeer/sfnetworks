...
In a tbl_graph structure, the weights of edges are normally stored in a column named *weight*. You can set edge weights yourself after construction (see the example in the [activation section](#activation-1)). For convenience, you can also tell the construction function to calculate the geographical lengths of the edges and set those as weight during construction.
...
Geometries can be replaced using either `sf::st_geometry(x) = value` or the pipe-friendly `sf::st_set_geometry(x, value)`. However, a replacement that breaks the valid spatial network structure will throw an error. 
