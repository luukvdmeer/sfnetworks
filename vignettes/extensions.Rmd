...
- `edge_displacement`: The straight-line distance (i.e. euclidean distance for projected coordinates, and great-circle-distance for geographic coordinates) between the two boundary nodes of an edge, as calculated by `st_distance`.
...
net %>%
  activate("edges") %>%
  mutate(len = edge_length(), dis = edge_displacement(), cir = edge_circuity()) %>%
  select(len, dis, cir)
```
...
```{r}
st_network_paths(net, 1, 9) %>% pull(node_paths) %>% unlist()
```
...
st_network_paths(net, p1, p2) %>% pull(node_paths) %>% unlist()
```
...
st_network_paths(net, p3, p4) %>% pull(node_paths) %>% unlist()
```

All paths above are calculated without taking into account weights of edges. Just as in `igraph::shortest_paths`, you can provide such weights through the weights argument. However, in `st_network_paths` you also have the option to just provide a column name to the weights argument, which should match an existing column in the networks edges table, whose values will then be used as weights.
...
st_network_paths(net, p1, p2, weights = "length") %>% pull(node_paths) %>% unlist()
```
...
Having multiple features as `from` argument ìs possible in `st_network_cost()`, which is a wrapper around `igraph::distances`. This returns a cost matrix.
...
st_network_cost(net, ps1, ps2)
```

Finally, `st_network_paths()` also wraps around `igraph::all_shortest_paths` by setting the argument `all = TRUE`. It works the same as `st_network_paths()` with `all = FALSE`, but in case there are multiple shortest paths from A to B, it will return all of them, instead of only one.
...
`to_spatial_subdivision` reconstruct the network by using all edge linestring points as nodes, instead of only the endpoints.

```{r, fig.show='hold'}
plot(net)
plot(convert(net, to_spatial_subdivision))
```
...
`to_spatial_subset` limits a graph to a single spatial subset. ... is evaluated in the same manner as `st_filter`.
