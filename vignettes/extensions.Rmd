...
```{r}
st_shortest_paths(net, 1, 9) %>% pull(node_path) %>% unlist()
```
...
st_shortest_paths(net, p1, p2) %>% pull(node_path) %>% unlist()
```
...
st_shortest_paths(net, p3, p4) %>% pull(node_path) %>% unlist()
```
...
st_shortest_paths(net, p1, p2, weights = "length") %>% pull(node_path) %>% unlist()
```
