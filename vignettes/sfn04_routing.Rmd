...
- Whenever there is no column in the edges table named *weight* and the `weights` argument is not set (i.e. `weights = NULL`), `st_network_paths()` by default uses the geographic edge length as weight.
...
In directed networks, `st_network_cost()` also gives you the possibility to define if you want to allow travel only over outbound edges (by setting `direction = "out"`, the default), only over inbound edges (by setting `direction = "in"`), or both (by setting `direction = "all"`, i.e. assume an undirected network).
The `direction` argument is passed onto `igraph::distances()` to the `mode` argument, and hence, if one passes the `mode` argument, it will be ignored. For undirected networks the `direction` argument is ignored.
