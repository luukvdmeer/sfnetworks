...
Instead of *from* and *to* columns containing integers that refer to node indices, the provided edges table can also have *from* and *to* columns containing characters that refer to node keys. In that case, you should tell the construction function which column in the nodes table contains these keys.
...
net_2 = sfnetwork(nodes, edges, edges_as_lines = TRUE)
...
nodes = st_as_sf(c(st_sfc(p1), st_sfc(p3), st_sfc(p4)), crs = 4326)

edges$from = 1
edges$to = 2

exnet = sfnetwork(nodes, edges)

plot(exnet)
plot(st_as_sfc(st_bbox(activate(exnet, "nodes"))), border = "red", lty = 2, add = TRUE)
plot(st_as_sfc(st_bbox(activate(exnet, "edges"))), border = "blue", lty = 2, add = TRUE)
plot(exnet)
plot(st_as_sfc(st_network_bbox(exnet)), border = "red", lty = 2, add = TRUE)
```
