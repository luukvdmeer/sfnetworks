...
## Installation
The `sfnetworks` package is still in very active development, and this vignette only described the first part of our work. Therefore, the package is also not on CRAN yet. Install it from GitHub using the `remotes` package in R.
...
- The object is a `tbl_graph` or convertable to a `tbl_graph` with `as_tbl_graph()`. See the [tidygraph documentation](https://tidygraph.data-imaginist.com/reference/tbl_graph.html) for all objects that are supported.
- At least the nodes are convertable to an `sf` object with `st_as_sf()`.

Additionally, `as_sfnetwork()` has a method that directly converts an `sf` object into a network. Of course, a single `sf` object does not contain any explicit network information, so assumptions have to made. This functionality should therefore be seen as a convenience function, and will not be suited for all possible use-cases. It works as follows:
...
- When given an `sf` object with `POINT` geometries: the points are considered nodes, and edges are created between then, in a sequential way. With that we mean: node 1 will be connected to node 2, node 2 to node 3, et cetera. This addresses the use-case that was presented in this [stackoverflow thread](https://stackoverflow.com/questions/57833905/how-to-convert-a-list-of-sf-spatial-points-into-a-routable-graph).
...
More methods will be added in the near future, such that other spatial network objects that exist in R can be directedly converted into an `sfnetwork`, e.g. `weighted_streetnet` from the `dodgr` package and `SpatialLinesNetwork` from the `stplanr` package.

### Activation and extraction
Just as with a `tbl_graph`, one of the graph elements of an `sfnetwork` (i.e. nodes or edges) can be activated, and all subsequent computations will be perfomed on that active element. By default, the nodes are the active element. This can be changed with the `activate()` verb, that is borrowed from `tidygraph`.

Thanks to `activate()`, a lot of data manipulation tools can be directly applied to the network. If you want to do something that goes furhter, you can always extract the active element as an `sf` object, simply by calling `st_as_sf()` - in the case of spatially implicit edges, extract them with `as_tibble()` instead.
...
## Spatial operations
Not only can `sfnetwork` objects be used in graph algorithms, they are also recognized by various spatial analytical functions of `sf`. Internally, this works as follows: the active element is converted to an `sf` object, to which the function is applied. Depending on the function, the results are merged back into the network, and an `sfnetwork` object is returned, or simply the output of the sf function is returned. 

Some of the functions have effects also outside of the active element: when nodes removed from the network, the edges terminating at those nodes will be removed too. This behavior is *not* symmetric: when removing edges, the nodes that are at the start or end of those edges remain, even if they don't connect to any other edge. This is because in graph theory, edges can never exist without end-nodes, while nodes can exist without having adjacent edges.
...
Geometries can replaced also by using [geometry unary operations](https://r-spatial.github.io/sf/reference/geos_unary.html), as long as they don't break the restrictions mentioned above. This basically means that only `st_reverse` and `st_simplify` are supported. In the case of `st_reverse`, the `to` and `from` column of the edges will be swapped as well.
...
```{r}
class(st_set_geometry(net, NULL))
```
...
For the same reason as `tidygraph` does not support all `dplyr` functions, `sfnetwork` does not support all `sf` functions: there is a clear limitation in the relational data structure that requires rows to maintain their identity. For `sfnetwork` objects, this also means that the geometries of features should be maintained, or at least the endpoints of these geometries, in the case of the edges. That is, functions that summarise geometries, or (may) change their *type*, *shape* or *position*, are not supported. These are the [geometric binary operations](https://r-spatial.github.io/sf/reference/geos_binary_ops.html), most of the [geometric unary operations](https://r-spatial.github.io/sf/reference/geos_unary.html), `st_union`, `st_combine`, `st_cast`, `st_nearest_point`, `st_sample`, `st_line_sample`, `st_jitter`, `st_collection_extract` and `st_make_valid`.
...
- All functions related to reading and writing data. Use `sf` for this, and then convert to an `sfnetwork`. (Note: Maybe there are specific spatial network data formats that the package should be able to read from and write to?)
- All functions related to M and Z coordinates. This is something for later.
- Functions that retrieve/change the attributes of sf objects. Like `st_agr`. See [this issue](https://github.com/luukvdmeer/sfnetworks/issues/24).
...
### Extending the functionalites of tidygraph and sf
