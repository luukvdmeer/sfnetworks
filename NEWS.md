...
* Functions `sf::st_precision()`, `sf::st_precision<-()` and `sf::st_set_precision()` now have a method for `sfnetwork` objects, such that coordinate precision can be queried and set.
* Updates to `st_network_cost()`:
  - Duplicated nodes are no accepted in the `to` argument. Refs [#183](
...
* By default `st_network_paths()` now encodes nodes by their name in the returned tibble, whenever a name attribute is present in the nodes table. This can be disabled by setting `use_names = FALSE`. Refs [#154](https://github.com/luukvdmeer/sfnetworks/issues/154).
* Functions `sf::st_intersection()` and `sf::st_difference()` now have a method for `sfnetwork` objects, such that networks can be spatially clipped. These functions do not work yet on edges of undirected networks.
...
* Error messages when edge measure or query functions are applied to a network with active nodes, as well as error messages when node measure or query functions are applied to a network with active edges, are now more informative. Refs [#216](https://github.com/luukvdmeer/sfnetworks/issues/216).
